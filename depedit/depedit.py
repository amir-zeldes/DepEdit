...
class Transformation:
    ...
    def __init__(self, transformation_text, line):
        ...
        if instructions is None:
            ...
            message += "Malformed instruction on line " + str(
                line) + " (instruction lines must contain exactly two tabs)\n"
    ...
    def validate(self):
        ...
        for relation in self.relations:
            if relation == "none" and len(self.relations) == 1:
                ...
                for criterion in criteria:
                    ...
                    if not re.match(
                            r"(#[0-9]+((>|\.([0-9]+(,[0-9]+)?)?)#[0-9]+)+|#[0-9]+:(text|pos|cpos|lemma|morph|func|head|func2|head2|num|form|upostag|xpostag|feats|deprel|deps|misc)==#[0-9]+)",
                            criterion):
        ...
        for action in self.actions:
            ...
            for command in commands:
                if not re.match(
                        r"(#[0-9]+>#[0-9]+|#[0-9]+:(func|lemma|text|pos|cpos|morph|head|head2|func2|num|form|upostag|xpostag|feats|deprel|deps|misc)=[^;]*)$",
                        command):  # Node action
...
class DepEdit():
                    ...
                    for key in solution:
                        if key != "rels" and key != "matchers":
                            if key in candidate:  # This is a position, e.g. #1, that is also in the candidate in merged
                                if solution[key] == candidate[
                                    key]:  # Check that they are compatible, e.g. #1 is the same token
                            ...
                            if group_num_matches is not None:
                                for g in group_num_matches:
                                    ...
                                    try:
                                        ...
                                        sys.stderr.write(
                                            "The action '" + action + "' refers to a missing regex bracket group '$" + str(
                                                group_num) + "'\n")
                                        sys.exit()
    ...
    def add_transformation(self, *args, **kwargs):
        ...
        if len(args) > 0:  # Single string transformation(s)
            ...
            if not (isinstance(kwargs["nodes"], list) and (isinstance(kwargs["nodes"], list)) and (
            ...
            if myline.startswith("#"):  # Preserve comment lines unless kill requested
                ...
                if "-" in cols[0]:  # potential conllu super-token, just preserve
                    ...
                    if cols[6] == "_":
                        if not self.quiet:
                            sys.stderr.write(
                                "DepEdit WARN: head not set for token " + tok_id + " in " + filename + "\n")
                ...
                if len(cols) > 8:
                    ...
                    this_tok = ParsedToken(tok_id, cols[1], cols[2], cols[3], cols[4], cols[5], head_id,
                                           cols[7].strip(), cols[8], cols[9].strip(), cols[0], [], "mid", super_tok)
                else:  # Attempt to read as 8 column Malt input
                    this_tok = ParsedToken(tok_id, cols[1], cols[2], cols[3], cols[4], cols[5], head_id,
                                           cols[7].strip(), cols[6], cols[7].strip(), cols[0], [], "mid", super_tok)
